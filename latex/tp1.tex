...
\For{$i \in$ conjAgentesSolución}
	\For{$j \in$ listaOpiniones}
		\If{$i$ opina de otro agente}
			\For{$k \in$ conjAgentesSolución}
				\If{$i$ opina de $k$} \Comment{$O(1)$}
					\State $i$ y $k$ estan en el conjunto solución
				\EndIf
			\EndFor
			
			\If{$i$ y $k$ estan en el conjunto solución AND $i$ no confia en $k$}
				\State Conjunto inconsistente, se descarta toda la rama
			\EndIf
			
			\If{$k$ no esta en el conjunto solución AND $i$ confia en $k$ AND $k$ ya no puede se agregada a la solución}
				\State Conjunto inconsistente, se descarta toda la rama
			\EndIf
			
			\If{$k$ no esta en el conjunto solución AND $i$ confia en $k$}
				\For{$l \in$ listaOpiniones}
					\If{Algún agente dice que $k$ no es confiable}
						\For{$m \in$ conjAgentesSolución}
							\If{$m$ dice que $k$ no es confiable}
